...
import in.abhi.service.AccountService;
...
public class AccountRestController {
	...
	public ResponseEntity<String> createAccount(@RequestBody UserAccForm userAccForm){
		...
		if(status) {
			return new ResponseEntity<>("Account creadted",HttpStatus.CREATED);
		}else {
			return new ResponseEntity<>("Account creadted failed",HttpStatus.INTERNAL_SERVER_ERROR);
		}
	}
	
	@GetMapping("/users")
	public ResponseEntity<List<UserAccForm>> getUser(){
